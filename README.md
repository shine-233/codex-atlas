...
| 线路 | 页面 | 回答什么 |
|---|---|---|
| 01 循环回路 | `labs/loop.html` | 一轮对话从 `codex` 命令到 TurnComplete 的 13 个阶段，单步可停；配会话建立段、重采样回路图（随阶段联动）、TURN SIMULATOR 沙盒（自己开一轮：发输入、调工具、运行中 steer 并入）与「先猜后揭」的成本对照 |
...
| 05 Crate 图谱 | `labs/atlas.html` | 固定提交下 codex-rs 的 135 个成员逐条注解，九条功能带 + 构成比例带 + 带间动线 + 检索高亮；可切「面积树图」视图——格子面积 = 该带成员数占比（squarified 布局），与检索、选中、详情卡全联动 |
| 06 术语速查 | `glossary.html` | 全站术语的一句话人话解释，可检索；正文里的虚线下划线词悬停即出卡片。另设「自测」模式：术语翻卡主动回忆，「记得 / 忘了」写入本机存档，忘了的卡自动沉到队尾重练 |
| 07 深水区 | `labs/deep.html` | 主干之外的五间房，各一台小仪器：A 凭据流选择器（ChatGPT 登录 vs API key，auth.json 形状与端点分岔）；B config 组装台 + 四层优先级阶梯（场景按钮现场演示谁覆盖谁）；C apply_patch 补丁解剖台（补丁可编辑、可应用、可故意弄坏看拒绝理由）；D TUI 拟态终端（点任意格子看驱动它的事件）+ 斜杠命令过滤面板；E 扩展机制挂点地图（skills / plugin / hooks / memories / ext 各挂主干哪一段，已知与未分明示） |
