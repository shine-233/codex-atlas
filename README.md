...
| 04 协议线路 | `labs/appserver.html` | App Server 双生命线时序图 + 报文时间线；审批请求怎样挂起整轮对话并分出批准 / 拒绝两条剧情。另有第二幕：resume / fork / interrupt / archive 四条支线时间线（断线重连、分叉、Esc 打断、归档），报文 JSON 带语法着色 |
| 05 Crate 图谱 | `labs/atlas.html` | 固定提交下 codex-rs 的 135 个成员逐条注解，九条功能带 + 构成比例带 + 带间动线 + 检索高亮；可切「面积树图」视图——格子面积 = 该带成员数占比（squarified 布局），与检索、选中、详情卡全联动；再切「3D 星系」视图——手写零依赖 3D 投影：九条带按球面金螺旋聚成星团，拖拽旋转（松手带惯性）、滚轮缩放锚定光标 / 右键平移 / 双击复位、闲置自转、悬停出名牌、点击选中进详情卡、点带名聚焦单带、自动导览模式，检索词实时暗化非命中点；固定种子生成，同基线下星图逐点可复现。另有「仓库全景」覆盖面树：本站管到哪一层、哪些明确不管 |
